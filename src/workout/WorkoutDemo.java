 ...
 * 
 * - Program is based off a yearly cycle. - Oct, Nov, Dec, Jan: (Goal: Strength)
 * 4x/week barbell strength. 2x/week LISS Cardio. - Feb, Mar: (Goal: Mass)
 * 6x/week Hypertrophy. PPLR PPLR. - Apr, May: (Goal: Fat Loss) 2x/week Barbell
 * Fullbody. 2x/week Running buildup. 4x/week HITT. - Jun, Jul, Aug, Sep: (Goal:
 * Running) 2x/week barbell upper/lower. 3+/week running.
 * 
 * - Demo will produce different workouts depending on the month of the year and
 * the day of the week
 * 
 * - For Wendler 531, it will ask the user if they have retested their maxes. It
 * stores this informtion in JSON files and uses it to calculate daily workouts.
 * 
 * - It will display the workout for the day using information stored in JSON
 * files: %s for Wendler, and randomly pulled metcons.
 * 
 * @author Josh
 * @version 8/18/18
 * 
...
public class WorkoutDemo {
	...
	public static void main(String[] args) {
		...
		/*
		 * 1. Ask if maxes were retested (This will be a button in Frame later) 2. If
		 * yes, ask for efforts. (weight, reps), calculate best effort. Store in JSON
		 * array. 3. If no, display workout of the day depending on month of year and
		 * day of week. 4. Ask, did you do this workout? 5. If yes, ask for AMRAP set
		 * reps. Store in JSON. Display past X (maybe 5) amraps from JSON to compare.
		 * Maybe all of the past AMRAPS, or 10, in descending order with dates. 6. If
		 * no, display a motivating message. 7. Ask, did you do the metcon (if there was
		 * one). 8. If yes, ask for metcon time/reps. Store in JSON. Display past
		 * performances with dates. 9. If no, display a motivating message.
		 */
		...
		System.out.println(
				"Today is: " + dayName + ", " + monthName + " " + date.getDayOfMonth() + ", " + date.getYear());
	...
	public void currentCycle(String monthName) {
		if (monthName.equals("May") || monthName.equals("June") || monthName.equals("July")
			...
			System.out.println("You are currently on the running cycle.");
		} else if (monthName.equals("September") || monthName.equals("October") || monthName.equals("November")
				|| monthName.equals("December")) {
			System.out.println("You are currently on the strength cycle.");
		} else if (monthName.equals("January") || monthName.equals("February")) {
			System.out.println("You are currently on the hypertrophy cycle.");
		} else if (monthName.equals("March") || monthName.equals("April")) {
			System.out.println("You are currently on the fat loss cycle.");
		}
	...
	public double[] newMaxes() {
		...
		System.out.println(
				"Once you decide to stop, we will compare all" + " results entered and determine your best effort.");
	...
	public void workout(String dayName, String monthName) {
		...
		/*
		 * Running LISS = 2x days/week Sprints = 2x days/week Metcons = 3x days/week
		...
		if (monthName.equals("May") || monthName.equals("June") || monthName.equals("July")
			...
			if (dayName.equals("Sunday")) {
				...
				// Workout work = new Workout(bench, upper);
		...
		/*
		 * Strength Training 4x days/week = 531 Upper/Lower, Upper/Lower LISS = 2x
		 * days/week Sept, Oct, Nov, Dec
		 */
		...
		/*
		 * Hypertrophy PPLPPLR Push1 Bench, Push2 OHP Pull1 Row, Pull2 Pullups Legs1
		 * Squat, Legs2 Deadlift Metcons 2x day/week after legs Incorporate running into
		 * every metcon
		 */
		...
		/*
		 * HITT/Running Buildup Barbell = 2x day/week full body Running = 2x day/week
		 * buildup MetCons = 4x day/week
		 */
		else if (monthName.equals("March") || monthName.equals("April")) {
